...
void Level3::OnStart()
{
    ...
    // Generate ground and enemies3 from level string
    float groundX = 0.0f;
	float groundY = 0.0f;
	int enemyCount = 0;
	for (int x = 0; x < width3; ++x)
	{
		...
		{
			if (level[y*width3+x] == '0') continue;
			if (level[y*width3+x] == '2')
			{
				m_Enemies[enemyCount].m_Position = { groundX + 32.0f * x, groundY - 32.0f * y};
				enemyCount++;
				continue;
			}
			m_GroundSprites[y*width3+x].SetTexture(level[y*width3 + x] == '1' ? "ground.png" : "dirt.png");
			m_GroundSprites[y*width3+x].m_Position = { groundX + 32.0f * x, groundY - 32.0f * y};
			m_GroundSprites[y*width3+x].AddStaticPhysics();
		}
	}
    ...
    // Add script to player and enemies3
	...
	for (int i = 0; i < enemies3; ++i) m_Enemies[i].m_bDrawCollider = true;
	for (int x = 0; x < width3*height3; ++x) m_GroundSprites[x].m_bDrawCollider = true;
#endif
...
int Level3::GetType(lua_State* L)
{
...
int Level3::OnUpdate()
{
	...
	// If all enemies3 hit, next level!
	bool x = true;
	for (int i = 0; i < enemies3; ++i)
	{
	...
	if (m_Player.m_Position.y < -100) return GAME_OVER;

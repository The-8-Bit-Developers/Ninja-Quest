...
void Level1::OnStart()
{
    ...
    // Generate ground and enemies11 from level string
    float groundX = 0.0f;
	float groundY = 0.0f;
	int enemyCount = 0;
	for (int x = 0; x < width1; ++x)
	{
		...
		{
			if (level[y*width1+x] == '0') continue;
			if (level[y*width1+x] == '2')
			{
				m_Enemies[enemyCount].m_Position = { groundX + 32.0f * x, groundY - 32.0f * y};
				enemyCount++;
				continue;
			}
			m_GroundSprites[y*width1+x].SetTexture(level[y*width1 + x] == '1' ? "ground.png" : "dirt.png");
			m_GroundSprites[y*width1+x].m_Position = { groundX + 32.0f * x, groundY - 32.0f * y};
			m_GroundSprites[y*width1+x].AddStaticPhysics();
		}
	}
    ...
    // Add script to player and enemies1
	...
	for (int i = 0; i < enemies1; ++i) m_Enemies[i].m_bDrawCollider = true;
	for (int x = 0; x < width1*height1; ++x) m_GroundSprites[x].m_bDrawCollider = true;
#endif
...
int Level1::GetType(lua_State* L)
{
...
int Level1::OnUpdate()
{
	...
	// If all enemies1 hit, next level!
	bool x = true;
	for (int i = 0; i < enemies1; ++i)
	{

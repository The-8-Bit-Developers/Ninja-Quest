...
void Level0::OnStart()
{
    ...
    // Generate ground and enemies0 from level string
    float groundX = 0.0f;
	float groundY = 0.0f;
	int enemyCount = 0;
	for (int x = 0; x < width0; ++x)
	{
		...
		{
			if (level[y*width0+x] == '0') continue;
			if (level[y*width0+x] == '2')
			{
				m_Enemies[enemyCount].m_Position = { groundX + 32.0f * x, groundY - 32.0f * y};
				enemyCount++;
				continue;
			}
			m_GroundSprites[y*width0+x].SetTexture(level[y*width0 + x] == '1' ? "ground.png" : "dirt.png");
			m_GroundSprites[y*width0+x].m_Position = { groundX + 32.0f * x, groundY - 32.0f * y};
			m_GroundSprites[y*width0+x].AddStaticPhysics();
		}
	}
    ...
    // Add script to player and enemies0
	...
	for (int i = 0; i < enemies0; ++i) m_Enemies[i].m_bDrawCollider = true;
	for (int x = 0; x < width0*height0; ++x) m_GroundSprites[x].m_bDrawCollider = true;
#endif
...
int Level0::GetType(lua_State* L)
{
...
int Level0::OnUpdate()
{
	...
	// If all enemies0 hit, next level!
	bool x = true;
	for (int i = 0; i < enemies0; ++i)
	{
